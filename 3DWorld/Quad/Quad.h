...
private:
    ...
    GLuint _rock;
    GLuint _water;
    const int _numOfVerts = 500; /// the number of triangles to render on a side (square terrain mesh)
    ...
    RGBImage makeHybridFBM(RGBImage perlin){
        ...
        for(int i = 0; i < perlin.rows(); i++){
            for(int j = 0; j< perlin.cols(); j++){
                ...
                result += perlin(x%perlin.rows(),y%perlin.cols());
                result(0) += offset;
                result(1) += offset;
                result(2) += offset;
                result *= expArray[0];
                ...
                for(int l = 0; l < MAX_OCTAVES; l++){
                    if(weight > 1.0)
                        weight = 1.0;
                    signal += perlin(x%perlin.rows(),y%perlin.cols());
                    signal(0) += offset;
                    signal(1) += offset;
                    signal(2) += offset;
                    signal *= expArray[l];
